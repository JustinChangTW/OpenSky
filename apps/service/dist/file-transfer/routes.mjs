...
import { ensureSiteForFeature, getRecord, isPreviewSupported, normalizePreviewPayload, withRoute, writeAudit, writeRecord } from "../common/service-helpers.mjs";
...
export function registerFileTransferRoutes(router) {
  router.add("POST", "/v1/file-transfer", withRoute(async ({ request, context, traceId, actor }) => {
    ...
    const itemId = await context.store.nextId("file");
    ...
    const site = await getRecord(context, "sites", item.siteId, ERROR_CODES.SITE_NOT_FOUND, traceId, "Site");
    ...
    await writeRecord(context, "fileTransfers", itemId, item);
    await writeAudit(context, { actorId: actor.actorId, action: "file-transfer.create", targetType: "FileTransferItem", targetId: itemId, result: "success" });
    return jsonResponse(item, { status: 201 });
  }));

  router.add("POST", "/v1/file-transfer/{itemId}/preview", withRoute(async ({ context, params, traceId, actor }) => {
    const currentItem = await getRecord(context, "fileTransfers", params.itemId, ERROR_CODES.INVALID_INPUT, traceId, "File transfer item");
    if (!isPreviewSupported(currentItem.mimeType)) {
      const failedItem = { ...currentItem, previewStatus: "failed", transferStatus: "failed" };
      await writeRecord(context, "fileTransfers", params.itemId, failedItem);
      await writeAudit(context, { actorId: actor.actorId, action: "file-transfer.preview", targetType: "FileTransferItem", targetId: params.itemId, result: "blocked" });
    ...
    await writeRecord(context, "fileTransfers", params.itemId, updatedItem);
    await writeAudit(context, { actorId: actor.actorId, action: "file-transfer.preview", targetType: "FileTransferItem", targetId: params.itemId, result: "success" });
  ...
  router.add("POST", "/v1/file-transfer/{itemId}/approve", withRoute(async ({ context, params, traceId, actor }) => {
    ...
    await writeRecord(context, "fileTransfers", params.itemId, updatedItem);
    await writeAudit(context, { actorId: actor.actorId, action: "file-transfer.approve", targetType: "FileTransferItem", targetId: params.itemId, result: "success" });
  ...
  router.add("POST", "/v1/file-transfer/{itemId}/complete", withRoute(async ({ context, params, traceId, actor }) => {
    ...
    await writeRecord(context, "fileTransfers", params.itemId, updatedItem);
    await writeAudit(context, { actorId: actor.actorId, action: "file-transfer.complete", targetType: "FileTransferItem", targetId: params.itemId, result: "success" });
    return jsonResponse(updatedItem);
  }));

  router.add("DELETE", "/v1/file-transfer/{itemId}", withRoute(async ({ context, params, traceId, actor }) => {
    const currentItem = await getRecord(context, "fileTransfers", params.itemId, ERROR_CODES.INVALID_INPUT, traceId, "File transfer item");
    await writeRecord(context, "fileTransfers", params.itemId, { ...currentItem, transferStatus: "deleted" });
    await writeAudit(context, { actorId: actor.actorId, action: "file-transfer.delete", targetType: "FileTransferItem", targetId: params.itemId, result: "success" });
