...
import { ensureSiteForFeature, getRecord, listRecords, maybeExpireVault, parseQuery, withRoute, writeAudit } from "../common/service-helpers.mjs";
...
export function registerSessionVaultRoutes(router) {
  router.add("GET", "/v1/session-vault", withRoute(async ({ context, url }) => {
    ...
    const items = listRecords(context.store.state.sessionVault)
      .map((item) => {
        const nextItem = maybeExpireVault(item);
        context.store.state.sessionVault.set(nextItem.vaultId, nextItem);
        return nextItem;
      })
  ...
  router.add("POST", "/v1/session-vault", withRoute(async ({ request, context, traceId, actor }) => {
    ...
    const vaultId = context.store.nextId("vault");
    const vault = { vaultId, ...payload };
    context.store.state.sessionVault.set(vaultId, vault);
    writeAudit(context, { actorId: actor.actorId, action: "session-vault.create", targetType: "ExternalSessionVault", targetId: vaultId, result: "success" });
  ...
  router.add("PATCH", "/v1/session-vault/{vaultId}", withRoute(async ({ request, context, params, traceId, actor }) => {
    const currentVault = getRecord(context.store.state.sessionVault, params.vaultId, ERROR_CODES.SESSION_VAULT_NOT_FOUND, traceId, "Session vault");
    ...
    context.store.state.sessionVault.set(params.vaultId, vault);
    writeAudit(context, { actorId: actor.actorId, action: "session-vault.update", targetType: "ExternalSessionVault", targetId: params.vaultId, result: "success" });
    return jsonResponse(vault);
  }));

  router.add("DELETE", "/v1/session-vault/{vaultId}", withRoute(async ({ context, params, traceId, actor }) => {
    const currentVault = getRecord(context.store.state.sessionVault, params.vaultId, ERROR_CODES.SESSION_VAULT_NOT_FOUND, traceId, "Session vault");
    context.store.state.sessionVault.set(params.vaultId, { ...currentVault, status: "deleted" });
    writeAudit(context, { actorId: actor.actorId, action: "session-vault.delete", targetType: "ExternalSessionVault", targetId: params.vaultId, result: "success" });
