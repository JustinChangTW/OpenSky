  ...
  const items = data.sites.length ? data.sites.map((site) => `
    <li class="data-list__item ${data.activeSiteId === site.siteId ? "is-active" : ""}">
      ...
      <div class="stack-actions">
        ...
        <button type="button" class="ghost-button" data-action="edit-site-url" data-site-id="${site.siteId}">${t("workspace.editSite")}</button>
...
function renderProjectList(data, t, locale) {
  if (data.projects.length <= 1) {
    ...
    const helperText = locale === "zh"
      ? "目前是單一工作區模式。大多數使用者不需要調整專案。"
      : "Single-workspace mode is active. Most users do not need project setup.";
    return `
      <section class="panel-section panel-section--advanced" id="settings-projects">
        <p class="eyebrow">${t("workspace.projects")}</p>
        ${project ? `
          <div class="data-list__item is-active">
            <div>
              <strong>${escapeHtml(project.name)}</strong>
              <span>${escapeHtml(translateEnumValue(project.status, locale))}</span>
            </div>
          </div>
        ` : `<p class="empty-state">${t("workspace.noProjects")}</p>`}
        <p class="panel-meta">${escapeHtml(helperText)}</p>
      </section>
    `;
  }
